...
import { isPasswordLoginAllowed } from './set-password.js';
...
router.get('/me-ext', (req, res) => {
    ...
    // req.user.profile is the full official record (incl. password hash), loaded by setUserDataMiddleware.
    // Metadata left behind by a deleted account that used the same handle is ignored.
    const meta = liveMetaForRecord(getUserMeta(handle), req.user.profile) || {};
    ...
    res.json({
        ...
        hasPassword: userHasKnownPassword(req.user.profile, meta),
...
router.post('/renew', (req, res) => {
    try {
        ...
        // A permanent account (expiresAt 0/unset) would be turned into an expiring one
        if (!getUserMeta(handle)?.expiresAt) {
            return res.status(400).json({ error: '账号已是永久有效，无需续费' });
        }
